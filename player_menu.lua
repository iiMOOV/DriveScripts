...
--=================================================================
-- [19] FEATURE: CHAT  (شات السيرفر + فتح بـ C)
--=================================================================
local chatStore = ac.storage{ chatKey = 67 } -- 67 = C
local chatMsgs = {}
local chatInputText = ""
local chatScrollDown = false
local chatPrevKey = false

ac.onChatMessage(function(message, author)
  table.insert(chatMsgs, { a = author, m = message })
  if #chatMsgs > 50 then table.remove(chatMsgs, 1) end
  chatScrollDown = true
end)

local function drawChat(X, Y, W, H)
  sectionTitle("الشات", "CHAT", X, Y, W)
  local inpH = 40
  local listY = Y + 44
  local listH = H - 44 - inpH - 42 -- نترك مساحة لزر المفتاح تحت
  ...
  ui.childWindow("##chatwin", vec2(W, listH), function()
    local cy = 4
    for i, msg in ipairs(chatMsgs) do
      ui.setCursor(vec2(8, cy))
      ui.pushDWriteFont(FONT)
      local isMe = msg.a == ac.getDriverName(0)
      ui.dwriteText(msg.a .. ": " .. msg.m, 14, isMe and CC or CW)
      ui.popDWriteFont()
      cy = cy + 22
    end
  ...
  -- صندوق الإدخال
  local inpY = listY + listH + 10
  local btnW = 80
  ui.setCursor(vec2(X, inpY + (inpH - 30) * 0.5))
  ui.setNextItemWidth(W - btnW - 10)
  ui.pushStyleColor(ui.StyleColor.FrameBg, rgbm(1, 1, 1, 0.05))
  chatInputText = ui.inputText("##cInp", chatInputText, ui.InputTextFlags.EnterReturnsTrue)
  local enter = ui.itemEdited()
  ui.popStyleColor()

  if bigButton(X + W - btnW, inpY, btnW, inpH, "إرسال", ACC, "##cSend") or enter then
    if chatInputText ~= "" then
      ac.sendChatMessage(chatInputText)
      chatInputText = ""
      chatScrollDown = true
    end
  end

  -- زر اختيار المفتاح الخاص بفتح الشات
  keyPicker(X, Y + H - 30, W, "مفتاح فتح الشات السريع", keyName(chatStore.chatKey), function()
    chatStore.chatKey = nextKey(chatStore.chatKey)
  end)
end
