...
local function drawTeleport(X, Y, W, H)
  ...
  ui.childWindow("##plist", vec2(W, listH), function()
    ...
    for i = 1, ac.getSim().carsCount - 1 do
      ...
      if c and c.isConnected and not c.isAIControlled and n ~= "DRIVE_ADMIN" and not string.find(n or "", "Traffic") then
        local ry = k * 46 + 4
...
local DriveChat = (function()
  ...
  for _, rx in ipairs({ "onnected", "joined the server", "left the server", "has left", "DRIVE_", "%$DRIVE" }) do
...
local function drawChatLog(sim)
  ...
  ui. -- (تم ترك هذا السطر مقطوعاً تماماً كما في رسالتك الأصلية للحفاظ على التطابق)
